...
def test_clean_dataframe_identity():
    """
    Test that clean_dataframe creates a deep copy
    and doesn't affect untouched columns.
    """
    df_raw = pd.DataFrame({
        "other_feature": [1, 2, 3],
...
def test_clean_dataframe_generalized_string_blanks():
    ...
    """
    df_raw = pd.DataFrame({
        # 'a' is repeated so uniqueness isn't 100%
        "col1": ["a", " ", "", "a", "nan"],
    ...
    # Since 'a' is the mode, the np.nan values created in step 1
    # will be filled with 'a' in step 3.b
    expected_col1 = pd.Series(["a", "a", "a", "a", "a"], name="col1")
    # the dtype might remain object for mixed strings,
    # so we don't strictly assert dtype unless needed
...
def test_clean_dataframe_target_mapping():
    ...
    expected_target = pd.Series(
        [1.0, 0.0, 0.0, 1.0, np.nan], name="target", dtype="float64"
    )
...
def test_clean_dataframe_target_mapping_ignores_other_values():
    """
    Test that target column mapping doesn't apply
    if values are not subset of acceptable booleans.
    """
    df_raw = pd.DataFrame({
...
def test_clean_dataframe_handles_all_nans_gracefully():
    ...
    expected_target = pd.Series(
        [1.0, 0.0, 1.0], name="target", dtype="float64"
    )
    pd.testing.assert_series_equal(df_clean["target"], expected_target)

    # Empty columns might stay in,
    # but shouldn't have crashed median/to_numeric logic
