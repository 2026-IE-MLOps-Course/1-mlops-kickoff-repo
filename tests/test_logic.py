...
print(
    "Subset check:",
    set(df_clean[target_column].dropna().unique()).issubset(
        {"Yes", "No", "True", "False"}
    )
)

is_text_type = (
    pd.api.types.is_object_dtype(df_clean[target_column])
    or pd.api.types.is_string_dtype(df_clean[target_column])
)
if is_text_type:
    is_subset = set(df_clean[target_column].dropna().unique()).issubset(
        {"Yes", "No", "True", "False"}
    )
    if is_subset:
